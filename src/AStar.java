...
public class AStar {
        ...
        public int copyPathFirstStep(int _x, int _y, int dx, int dy) { // return a digit : 2 4 6 8 
        	Node node = map.getNode(_x, (_y-1+dy)%dy);
        	if (shortestPath.contains(node.getX(), node.getY()))
        		return 8;
        	node = map.getNode(_x, (_y+1+dy)%dy);
        	if (shortestPath.contains(node.getX(), node.getY()))
        		return 2;
        	node = map.getNode((_x-1+dx)%dx, _y);
        	if (shortestPath.contains(node.getX(), node.getY()))
        		return 4;
        	node = map.getNode((_x+1+dx)%dx, _y);
        	if (shortestPath.contains(node.getX(), node.getY()))
